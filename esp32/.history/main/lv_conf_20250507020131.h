...
/**创建OBJ时自动分配ID*/
/**使用内置的OBJ ID处理程序功能：
*-LV_OBJ_ASSIGN_ID：创建小部件时调用。使用每个小部件类的单独计数器作为ID。
*-LV_OBJ_ID_COMPARE：比较ID以确定其是否与请求的值匹配。
*-LV_OBJ_STRINGIFY_ID：返回字符串式标识符，例如“ button3”。
*-LV_OBJ_FREE_ID：没有做任何事情，因为ID没有内存分配。
*禁用后，这些功能需要由用户实现。*/
