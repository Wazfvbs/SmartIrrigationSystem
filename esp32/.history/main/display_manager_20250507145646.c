...
#define lv_mutex_lock() esp_lvgl_port_lock(0)
#define lv_mutex_unlock() esp_lvgl_port_unlock()
...
esp_err_t display_manager_show_env(const sensor_data_t *data)
{
    ...
    snprintf(buffer, sizeof(buffer),
             ...
             data->water_level,
             data->battery_level);
    ESP_LOGI(TAG, "Updating screen with temperature=%.1f", data->env_temperature);
