...
#define MQTT_BROKER_URI "mqtt://192.168.2.134" // 树莓派服务器局域网IP，改成你的
...
static void mqtt_event_handler(void *handler_args, esp_event_base_t base, int32_t event_id, void *event_data)
{
    ...
    case MQTT_EVENT_DATA:
        ...
        command_manager_handle(event->topic, event->data, event->data_len);
        break;
    default:
        break;
    }
...
void mqtt_client_start(void)
{
    const esp_mqtt_client_config_t mqtt_cfg = {
        .broker.address.uri = "mqtt://192.168.2.134", // 必须是 mqtt://，不是 mqtts://
        .broker.address.port = 1883,                  // 明确指定1883端口
    };
    ...
    ESP_LOGI("MQTT", "Connecting to broker at: %s", MQTT_BROKER_URI);
}
